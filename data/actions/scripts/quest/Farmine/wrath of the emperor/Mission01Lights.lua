...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if(item.uid == 3171) then
		if Game.getStorageValue(GlobalStorage.WrathOfTheEmperor.Light01) ~= 1 then
			...
			addEvent(Game.setStorageValue, 60 * 1000, GlobalStorage.WrathOfTheEmperor.Light01, 0)
			local pos = {
			...
			for i = 1, #pos do
				...
				addEvent(transformLamp, 60 * 1000, pos[i], 11446, 11447)
			end
		end
	elseif(item.uid == 3172) then
		if Game.getStorageValue(GlobalStorage.WrathOfTheEmperor.Light02) ~= 1 then
			...
			addEvent(Game.setStorageValue, 60 * 1000, GlobalStorage.WrathOfTheEmperor.Light02, 0)
			local pos = Position(33360, 31079, 8)
			transformLamp(pos, 11449, 11463)
			addEvent(transformLamp, 60 * 1000, pos, 11463, 11449)
		end
	elseif(item.uid == 3173) then
		if Game.getStorageValue(GlobalStorage.WrathOfTheEmperor.Light03) ~= 1 then
			...
			addEvent(Game.setStorageValue, 60 * 1000, GlobalStorage.WrathOfTheEmperor.Light03, 0)
	...
	elseif(item.uid == 3174) then
		if Game.getStorageValue(GlobalStorage.WrathOfTheEmperor.Light04) ~= 1 then
			...
			addEvent(Game.setStorageValue, 60 * 1000, GlobalStorage.WrathOfTheEmperor.Light04, 0)
			local wallItem, pos
			for i = 1, 4 do
				...
				if wallItem then
					wallItem:remove()
					addEvent(Game.createItem, 60 * 1000, 9264, 1, pos)
				end
			end
		end
	end
