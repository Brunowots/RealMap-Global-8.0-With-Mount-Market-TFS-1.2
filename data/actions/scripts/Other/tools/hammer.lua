...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if targetActionId == 50109 then
		if player:getItemCount(5901) >= 3 and player:getItemCount(8309) >= 3 then
			...
			local riverPosition = Position(32571, 31508, 9)
			local stoneItem = Tile(riverPosition):getItemById(7644)
				if stoneItem then
				stoneItem:remove()
				end
			local stoneItem = Tile(riverPosition):getItemById(7642)
				if stoneItem then
				stoneItem:remove()
				end
			...
			player:say("You need nails and wood to fix it..", TALKTYPE_MONSTER_SAY)
		end
