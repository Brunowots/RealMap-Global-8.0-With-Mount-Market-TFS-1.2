...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
	...
	if (scripted) {
		...
		CombatType_t combatType = params.combatType;
		CombatType_t ElementType = getElementType();



		if (combatType == 8192) { // significa que eh mix das weapons.xml

			int32_t mix_storage;
			player->getStorageValue(32457, mix_storage);

			if (mix_storage == 1) {
				combatType = COMBAT_ICEDAMAGE;
			}
			else if (mix_storage == 2) {
				combatType = COMBAT_FIREDAMAGE;
			}
			else if (mix_storage == 3) {
				combatType = COMBAT_EARTHDAMAGE;
			}
			else if (mix_storage == 4) {
				combatType = COMBAT_ENERGYDAMAGE;
			}
			else if (mix_storage == 5) {
			...
			//std::cout << "mix_storage: " << mix_storage << std::endl;
		}
		else if (ElementType == 8192) { // significa que eh mix dos items.xml
		...
		// repair de weapons element damage
		if ((item->getWeaponType() == WEAPON_AMMO) && getElementType()) {
			float ElementalAttack = getElementDamage(player, target, item);
			ElementalAttack = (ElementalAttack / 100);
			ElementalAttack = ElementalAttack * damage.primary.value;
			damage.primary.value = 0;
			damage.secondary.value = ElementalAttack;
		}
		else {
			float ElementalAttack = getElementDamage(player, target, item);
			ElementalAttack = (ElementalAttack / 100);
		...
		//std::cout << "Dano primario: " << damage.primary.value << "." << std::endl;
		//std::cout << "Dano primario tipo : " << damage.primary.type << "." << std::endl;

		//std::cout << "Dano secundario: " << damage.secondary.value << "." << std::endl;
		//std::cout << "Dano secundario tipo : " << damage.secondary.type << "." << std::endl;


		//std::cout << "combatType: " << params.combatType << "." << std::endl;
		//std::cout <<  "item:getWeaponType: " << item->getWeaponType() << std::endl;
		//std::cout << "getElementType: " << getElementType() << "." << std::endl;
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	if (item->hasAbility(ITEM_ABILITY_ELEMENTDAMAGE)) {
		uint64_t elementDamageValue = item->getAbilityInt(ITEM_ABILITY_ELEMENTDAMAGE);
		attackValue += elementDamageValue;
	}
...
bool WeaponWand::configureEvent(const pugi::xml_node& node)
{
	...
	else if (tmpStrValue == "mix") {
		params.combatType = COMBAT_MIX;
	}
	else {
