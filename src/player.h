...
class Player final : public Creature, public Cylinder
{
	public:
		...
		LightInfo getCreatureLight() const;
	...
	protected:
		...
		// attack speed storage change
		uint32_t getAttackSpeed() const {
			uint32_t FAST_ATTACK_SPEED = g_config.getNumber(ConfigManager::FAST_ATTACK_SPEED);
			float SpeedAttack = FAST_ATTACK_SPEED;
			float extra_speed = 1.0f;

			// Helper function to apply the bonus speed reduction based on a storage value
			auto applySpeedReduction = [&](int32_t value) {
				if (value > 0) {
					if (value >= 10) {
						extra_speed -= 0.1f;
					}
						...
						extra_speed -= test / 100.0f;
					}
				}
				};

			int32_t value;
			this->getStorageValue(87130, value);
			applySpeedReduction(value);

			int32_t value2;
			this->getStorageValue(87131, value2);
			applySpeedReduction(value2);

			int32_t value3;
			this->getStorageValue(87132, value3);
			applySpeedReduction(value3);

			// Calculate the bonusPercent based on getEffectiveAbility(ITEM_ABILITY_ATTACKSPEED)
			int64_t bonusPercent = getEffectiveAbility(ITEM_ABILITY_ATTACKSPEED);

			// Calculate the final attack speed including bonusPercent
			SpeedAttack *= (1.0f - bonusPercent / 100.0f);

			// Apply the extra_speed reduction
			SpeedAttack *= extra_speed;

			return static_cast<uint32_t>(SpeedAttack);
		}
